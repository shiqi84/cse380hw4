                ...
                case HW4_Events.PLAYER_HIT_COIN:
                    {
                        ...
                        // let coinTimer= new Timer(300, () => {
                        //    let tmp;
                        //    if(event.data.get("node") === this.player.id){
                        //     // Other is coin, disable
                        //     tmp = this.sceneGraph.getNode(event.data.get("other"));
                        // } else {
                        //     // Node is coin, disable
                        //     tmp = this.sceneGraph.getNode(event.data.get("node"));
                        // }   tmp.visible=false;
                        ...
                        //         {
                        //             property: "positionY",
                        //             start: coin.position.y,
                        //             end:coin.position.y-200,
                        //             ease: EaseFunctionType.IN_OUT_QUAD
                        //         }
                        //     ]
                        // });
                        // coinTimer.reset();
                        // if(!coinTimer.hasRun() && coinTimer.isStopped()){
             
                        //     coinTimer.start();
                        //     coin.tweens.play("die");
    ...
    protected handlePlayerEnemyCollision(player: AnimatedSprite, enemy: AnimatedSprite) {
      ...
      if(enemy.imageId=="GhostBunny"){
          if(player.position.y-enemy.position.y<-16){
              enemy.destroy();
              this.emitter.fireEvent(GameEventType.PLAY_SOUND, {key: "enemy_die", loop: false, holdReference: false});
          }else{
            if(!this.respawnTimer.hasRun() && this.respawnTimer.isStopped()){
             
                this.respawnTimer.start();
                this.player.tweens.play("die")
                this.player.freeze();
